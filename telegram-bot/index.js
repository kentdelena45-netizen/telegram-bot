...
bot.onText(/\/open/, (msg) => {
    ...
    shopStatus = 'open';
    bot.sendMessage(msg.chat.id, "ðŸŸ¢ Shop is now OPEN!");
});
...
bot.onText(/\/close/, (msg) => {
    if (msg.from.id !== OWNER_ID) return;
    shopStatus = 'closed';
    bot.sendMessage(msg.chat.id, "ðŸ”´ Shop is now CLOSED!");
});
...
bot.on('message', (msg) => {
    ...
    if (text === "ðŸ“Š Check Status") {
        if (shopStatus === 'open') {
            return bot.sendMessage(chatId, "ðŸŸ¢ Shop is OPEN!");
        } else {
            return bot.sendMessage(chatId, "ðŸ”´ Shop is CLOSED!");
        }
    }

    // BUY BUTTON
    if (text === "ðŸ›’ Buy Account") {
        if (shopStatus === 'closed') {
            return bot.sendMessage(chatId, "ðŸ”´ Shop is CLOSED. Try later.");
        }
    ...
    if (userState[chatId]) {
        ...
        if (state.step === 'details') {
            ...
            bot.sendMessage(chatId, "âœ… Order sent! Wait for admin reply.");
